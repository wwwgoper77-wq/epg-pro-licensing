...
# Compatible with Python 2 and Python 3 Enigma2 (OpenATV, OpenPLi, Egami, BlackHole, DreamOS, etc.)
...
try:
    ...
    from Components.config import config, ConfigText, ConfigSelection, getConfigListEntry, ConfigSubsection, configfile
    ...
    class MockConfigPlugin(object):
        def __init__(self):
            self.license_key = type('MockConfigText', (object,), {'value': "EPG-PRO-LIFETIME"})()
            self.engine = type('MockConfigSelection', (object,), {'value': "gemini"})()
    class MockConfig(object):
        def __init__(self):
            self.plugins = type('MockPlugins', (object,), {'arabic_epg': MockConfigPlugin()})()
    ...
    class Label(object):
        ...
        def setText(self, text):
            self.text = text
    def PluginDescriptor(*args, **kwargs): pass

# Constants
LICENSE_SERVER = "https://epg-pro-licensing-production.up.railway.app" # Production URL
SECRET_SALT = "EPG_PRO_SECURE_SALT_9933" # Matches SIGNATURE_SECRET in server.ts and SECRET_SALT in obfuscate.py
...
def get_hardware_id():
    ...
    raw_hwid = "ArabicEPG-%s-%s" % (machine_id, mac)
    if sys.version_info[0] >= 3:
        return hashlib.sha256(raw_hwid.encode('utf-8')).hexdigest()[:16].upper()
    else:
        return hashlib.sha256(raw_hwid).hexdigest()[:16].upper()
...
def verify_license_signature(key, hwid, expires_at, signature):
    """Verify server HMAC-SHA256 signature. Must match server.ts generateHmacSignature."""
    if not key or not hwid or not expires_at or not signature:
        return False
    raw_signature = "%s:%s:%s" % (key, hwid, expires_at)
    if sys.version_info[0] >= 3:
        expected = hmac.new(SECRET_SALT.encode('utf-8'), raw_signature.encode('utf-8'), hashlib.sha256).hexdigest()
    else:
        expected = hmac.new(SECRET_SALT, raw_signature, hashlib.sha256).hexdigest()
    try:
...
def check_license_status():
    ...
    try:
        url = "%s/api/license/verify?key=%s&hwid=%s" % (LICENSE_SERVER, key, hwid)
        req = Request(url)
        req.add_header('User-Agent', 'Enigma2/ArabicEPGTranslator')
        ...
        if verify_license_signature(key, hwid, expires_at, signature):
            ...
            if expires_at != "lifetime":
                try:
                    from datetime import datetime
                    exp_clean = expires_at.split('.')[0].replace('Z', '').replace('T', ' ')
                    exp_dt = datetime.strptime(exp_clean, '%Y-%m-%d %H:%M:%S')
                    if exp_dt < datetime.utcnow():
            ...
            if elapsed_seconds <= (grace_days * one_day):
                ...
                return "OfflineGrace", "Offline Grace Period active (%d days remaining)." % days_left
            else:
...
if ENIGMA_AVAILABLE:
    ...
    class ArabicEPGTranslatorConfigScreen(ConfigListScreen, Screen):
        ...
        def __init__(self, session):
            ...
            self["sep"] = Label("")
            self["sep2"] = Label("")
            self["hwid_label"] = Label("Receiver Hardware ID (HWID): %s" % get_hardware_id())
        ...
        def update_license_display(self):
            ...
            display_text = "Status: %s\nDetails: %s" % (status_code.upper(), status_msg)
        ...
        def keyActivate(self):
            ...
            # Build request
            try:
                url = "%s/api/license/activate" % LICENSE_SERVER
                post_data = urlencode({
                    'key': license_key,
                    'hwid': hwid,
                    'image': image,
                ...
                if res_body.get("success"):
                    ...
                    self["status_label"].setText("Status: ACTIVATION FAILED\n%s" % error_msg)
                ...
                self.session.open(MessageBox, "Licensing Server Connection Error:\n%s\n\nPlease check your internet connection." % err_str, MessageBox.TYPE_ERROR)
                self.update_license_display()
